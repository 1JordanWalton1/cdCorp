...
function initSliders() {
  if (document.querySelector(".hero__slider")) {
    new Swiper(".hero__slider", {
      ...
      slidesPerView: 1,
      spaceBetween: 0,
      // parallax: true, // Если нужно
      //autoHeight: true,
      speed: 800,
      // Оставляем fade, как вы хотите
      // effect: 'fade',
      fadeEffect: {
        crossFade: true
        // Плавный fade
      ...
      // Кнопки "вліво/вправо"
      navigation: {
        prevEl: ".slide-hero-button-prev",
        nextEl: ".slide-hero-button-next"
      },
      // Події
      on: {}
    });
  ...
  if (document.querySelector(".quote__slider")) {
    new Swiper(".quote__slider", {
      ...
      // effect: 'fade',
      // fadeEffect: {
      // 	crossFade: true  // Плавный fade
      // },
      //touchRatio: 0,
      //simulateTouch: false,
      // loop: true, // Включили loop
      //preloadImages: false,
      // lazy: true,
      // autoplay: {
      // 	delay: 3000,
...
document.addEventListener("DOMContentLoaded", () => {
  const player = document.getElementById("myVideoBlock");
  const video = player.querySelector(".player-video__video");
  const poster = player.querySelector(".player-video__poster");
  const btn = player.querySelector(".player-video__play");
  const showOverlay = () => player.classList.remove("is-playing");
  const hideOverlay = () => player.classList.add("is-playing");
  function togglePlay() {
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".form-feedback__textarea").forEach((wrap) => {
    const area = wrap.querySelector(".form-feedback__input--textarea");
    const handle = wrap.querySelector(".resize-custom");
    const minH = 43;
    positionHandle();
    ...
    function positionHandle() {
      const offset = 4;
      const top = area.offsetHeight - handle.offsetHeight - offset;
      handle.style.top = `${top}px`;
    }
    area.addEventListener("input", positionHandle);
  });
});
